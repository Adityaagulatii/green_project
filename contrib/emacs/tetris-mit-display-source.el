...
;; This is the source side of the user's display protocol, the
;; wal.sh/tools/display relay (see
;; /scratch/work/tetris-parallel/inputs/wal-sh-display-protocol.md).
;; Emacs opens a WebSocket to a relay and sends a `reserve'.  Once the
;; lease is granted, it sends binary frames.  A frame is w*h*3 bytes of
;; row-major RGB, with row 0 at the top and column 0 at the left, and an
;; optional 2-byte big-endian sequence number in front.  Emacs sends
;; `renew' when it is idle, and `release' on quit or kill-buffer.
;;
;; There are two frame sources:
;; - `tetris-mit-display-source-game' sends our game frames, whatever
;;   the overlay display (`tetris-mit-display') is showing, whether they
;;   come from the engine server or from an in-process provider.
...
(defcustom tetris-mit-display-source-url "ws://127.0.0.1:8080/tools/display/ws"
...
(defcustom tetris-mit-display-source-display "green-building"
  "The display to reserve: a d= profile name, such as tetris or trs80."
  :type 'string)
...
(defcustom tetris-mit-display-source-sequence nil
  "Non-nil prefixes each frame with a 2-byte big-endian sequence number."
  :type 'boolean)

(defconst tetris-mit-display-source-profiles
  '(("tetris" 10 20) ("green-building" 9 17) ("dc32" 10 18) ("gameboy" 10 18)
    ("trs80" 10 12) ("c64" 10 20) ("ws2812" 16 16) ("hub75" 64 32)
    ("remote" 9 17))
  "The user's d= display profiles, as (NAME W H).
...
(cl-defstruct (tetris-mit-display-source
  ...
  link (state 'idle) name display ttl (w 0) (h 0) (fps 30) lease expires holder
  last-error (last-sent 0.0) (last-activity 0.0) (seq 0) (sent 0) (dropped 0)
...
(defun tetris-mit-display-source-reserve-message (name display ttl)
  "The `reserve' text for NAME, DISPLAY and TTL, which is clamped to 1..900."
  (json-encode `((op . "reserve") (name . ,name) (display . ,display)
                 (ttl . ,(max 1 (min tetris-mit-display-source-max-ttl ttl))))))

(defun tetris-mit-display-source-pack (rows w h)
  "ROWS centered on a W x H black canvas, as a W*H*3 unibyte string.
ROWS is a vector of rows (top first) of [R G B] vectors.  A frame
larger than the canvas is cropped around its center.  The bytes are
row-major RGB, with row 0 at the top and column 0 at the left."
  (let* ((rh (length rows))
    ...
    (dotimes (y h)
      (let* ((sy (- y dy))
        ...
        (dotimes (x w)
          (let* ((sx (- x dx))
                 (cell (and row (>= sx 0) (< sx rw) (aref row sx))))
            (if cell
                (setq bytes (cons (aref cell 2) (cons (aref cell 1)
                                                      (cons (aref cell 0) bytes))))
              (setq bytes (cons 0 (cons 0 (cons 0 bytes)))))))))
...
(cl-defun tetris-mit-display-source-open (display &key url link name ttl provider)
...
WebSocket, as the tests do.  NAME and TTL default to the options.
  ...
  (let ((source (tetris-mit-display-source--make
    ...
    (funcall (tetris-mit-display-source-link-send-text
             ...
             (tetris-mit-display-source-reserve-message
              ...
              (tetris-mit-display-source-ttl source)))
    source))
...
(defun tetris-mit-display-source-receive (source text)
  ...
  (let ((msg (condition-case nil (tetris-mit--json-parse text) (error nil))))
    (pcase (and (consp msg) (consp (car msg)) (alist-get 'op msg))
      ...
      ("error"
       (setf (tetris-mit-display-source-last-error source) (alist-get 'reason msg))
       (when (equal (alist-get 'reason msg) "not holder")
         (setf (tetris-mit-display-source-state source) 'lost)
         (tetris-mit-display-source--cancel-timers source))
       (message "Display relay error: %s" (alist-get 'reason msg))))))
...
(defun tetris-mit-display-source--send-now (source frame now)
  ...
  (let* ((rows (if (functionp frame) (funcall frame) frame))
         (bytes (tetris-mit-display-source-pack rows (tetris-mit-display-source-w source)
                                                (tetris-mit-display-source-h source))))
    (when tetris-mit-display-source-sequence
      (let ((seq (tetris-mit-display-source-seq source)))
        (setq bytes (concat (unibyte-string (logand (ash seq -8) 255) (logand seq 255))
                            bytes))
        (setf (tetris-mit-display-source-seq source) (logand (1+ seq) #xffff))))
    (funcall (tetris-mit-display-source-link-send-binary
              (tetris-mit-display-source-link source))
             bytes)
...
display's w x h grid, from `granted'.  URL defaults to
`tetris-mit-display-source-url'.  LINK is for tests.  Killing the
